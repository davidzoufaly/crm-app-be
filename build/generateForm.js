const generateForm = (req) => {
    ...
    req.map((e, i) => {
        if (e.fieldInForm) {
            if (e.fieldType === "text" || e.fieldType === "number") {
                ...
                inputs.push(`var inputWrapper${i} = document.createElement('DIV');
            ...
            inputWrapper${i}.appendChild(crmInput${i})`);
                ...
                if (e.fieldFormVisible) {
                    ...
                    const optionsSelect = [];
                    e.fieldOptions.map((option, index) => {
                        optionsSelect.push(`var option${index} = document.createElement('OPTION');
              ...
              select${i}.appendChild(option${i});
                ${optionsSelect.join(";")}`);
                    ...
                    const hiddenPreSelectedOption = e.fieldOptions.filter((option) => option.preselected);
    ...
    const styling = [
        ...
        },
        {
            ...
            styles: 'width: 100%; height: 2rem; outline: none; border: 1px solid black; background-color: rgb(248, 248, 248);font-size: 1rem; height: 2rem; text-transform: uppercase'
        },
        {
